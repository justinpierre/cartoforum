...
from cartoforum_api.orm_classes import sess, Users, Post, Votes
...
@carto.route('/_save_object', methods=['POST', 'GET'])
def save_object():
    geom = request.args.get('jsonshp', 0, type=str)
    geom = unquote(geom)
    
    cur.execute("INSERT INTO mapobjects (geom, groupid, userid, date) VALUES (ST_GeomFromText('GEOMETRYCOLLECTION({})',3857), {}, {}, "
                "'{}');".format(geom, session['groupid'], session['userid'], datetime.datetime.utcnow()))
    pgconnect.commit()
    query = cur.execute("SELECT gid FROM mapobjects WHERE userid = {0} AND date = (SELECT max(date) "
...
@carto.route('/_zoom_to', methods=['GET'])
def zoom_to():
    bbox = ''
...
@carto.route('/discovery', methods=['POST'])
def go_to_disc():
    return render_template('discovery.html')


@carto.route('/viewmap', methods=['GET'])
def readonly_view():
...
@carto.route('/map', methods=['POST','GET'])
def go_to_group():
    try:
